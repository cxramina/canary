...
function SubHeader({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <header className={cn('bg-background-1 fixed left-[220px] right-0 top-[55px] z-40 h-[45px]', className)}>
      {children}
    </header>
  )
}
...
function Content({ children, maxWidth, className, paddingClassName = 'px-6 py-5 pb-24' }: ContentProps) {
